...
def batch_intersection_union(output, target, nclass):
    ...
    area_inter = intersection  # np.histogram(intersection, bins=nbins, range=None)
...
def batch_tp_fp_fn(output, target, nclass):
    ...
    target = target.cpu().numpy().astype('int64')  # T
    intersection = predict * (predict == target)  # TP
    print("intersection_none:",intersection.nonzero())
    # areas of intersection and union
    area_inter, _ = np.histogram(intersection, bins=nbins, range=(1, 1))
    area_pred, _ = np.histogram(predict, bins=nbins, range=(1, 1))
    area_lab, _ = np.histogram(target, bins=nbins, range=(1, 1))
    print('area_pred:',area_pred[0])
    print('area_lab:',area_lab[0])


    # areas of TN FP FN
    area_tp = area_inter[0]
    area_fp = np.abs(area_pred[0] - area_inter[0])
    area_fn = np.abs(area_lab[0] - area_inter[0])

    # area_union = area_pred + area_lab - area_inter
    assert area_tp <= (area_tp + area_fn + area_fp)
    print("area_tp:",area_tp)
    print("area_fp:",area_fp)
    print("area_fn:",area_fn)
...
def cal_tp_pos_fp_neg(output, target, nclass, score_thresh):
    ...
    maxi = nclass  # nclass
    nbins = nclass  # nclass
    #b = torch.sigmoid(output)
    predict = (torch.sigmoid(output).cpu().detach().numpy() >score_thresh).astype(
        'int64')  # P should rewrite this line when measure Fa and Pd
